...
def source_counts(collection) -> dict[str, int]:
    """How many chunks each ingested document holds, keyed by file name.
...
def delete_source(collection, source: str) -> int:
    """Drop every chunk that came from one document. Returns how many went.

    The count is taken before the delete because Chroma's `delete` returns
    nothing useful, and "removed 0 chunks" is the answer a caller needs when it
    asked about a document that was never ingested.
    """
